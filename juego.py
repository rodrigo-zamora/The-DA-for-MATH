...
def loadImage(filename, transparent=False):
    ...
    #Asignar la variable a la imagen
    image = pygame.image.load(filename) 

    #Convertir la imagen
    image = image.convert() 
    
    #Condición para hacer transparente la imagen
    if transparent: 
        
        #Obtener el color del punto 0, 0 en X, Y
        color = image.get_at((0,0))

        #
        image.set_colorkey(color, RLEACCEL)

        #Transformar el avatar a 80x80
...
def gameLost():
    ...
    while gameLost == True:
        ...
        textoLost = font.render("¡Has perdido!", True, black)
        textoLost_rect = textoLost.get_rect()
        textoLost_rect.center = (int(screenWidth/2), 200)
        screen.blit(textoLost, textoLost_rect)
        texto_puntajeTotal = font.render(f"Nivel alcanzado: {nivel}", True, black)
        texto_puntajeTotal_rect = texto_puntajeTotal.get_rect()
        texto_puntajeTotal_rect.center = (int(screenWidth/2), 250)
        screen.blit(texto_puntajeTotal, texto_puntajeTotal_rect)

        texto_puntajeTotala = font.render("¿Quieres volver a jugar?", True, black)
...
def gameMenu():
    ...
    while gameMenu == True:
        ...
        text = font.render("  Haz click aquí para empezar a jugar  ", True, black)
        text_rect = text.get_rect()
        text_rect.center = (int(screenWidth/2), 100)

        #Crear ventana del menú del juego
        displaySurface = pygame.display.set_mode((screenWidth, screenHeight))

        #Rellenar la ventana de color blanco
        displaySurface.fill(white)

        #Mostrar el texto con su rectángulo
...
def main():
    ...
    #Poner el flag gameRunning en verdadero
    gameRunning = True
    ...
    while gameRunning == True:
        ...
        #Poner toda la pantalla en blanco
        screen.fill(white)
        ...
        textoNivel = font.render(f"Problema {nivel}", True, black)
        ...
        pista = font16.render("Mueve el personaje hacia la respuesta correcta", True, yellow)
        ...
        pista_rect.center = (int((screenWidth/2)), 675)
